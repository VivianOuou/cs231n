...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        list = [input_dim] + hidden_dims + [num_classes]

        for i in range(self.num_layers):
          self.params[f'W{i+1}'] = np.random.randn(list[i],list[i+1]) * weight_scale
          self.params[f'b{i+1}'] = np.zeros(list[i+1])

          if self.normalization and i < self.num_layers-1:
            self.params[f'gamma{i+1}'] = np.ones(list[i+1])
            self.params[f'beta{i+1}'] = np.zeros(list[i+1])
    ...
    def loss(self, X, y=None):
        ...
        for i in reversed(range(self.num_layers-1)):
          ...
          dout = relu_backward(dout,relu_cache)
           ...
           #batch nomalization
          if self.normalization == "batchnorm":
            dout,grads[f'gamma{i+1}'],grads[f'beta{i+1}'] = batchnorm_backward(dout,bn_cache)

          #layer normalization
          if self.normalization == "layernorm":
            dout,grads[f'gamma{i+1}'],grads[f'beta{i+1}'] = layernorm_backward(dout,bn_cache)


           #affine
          dout,grads[f'W{i+1}'],grads[f'b{i+1}'] = affine_backward(dout,fc_cache)
