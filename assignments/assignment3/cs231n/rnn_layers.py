...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      ...
      dx_step, dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dnext_h, cache[t])
...
def word_embedding_backward(dout, cache):
    ...
    #以x作为索引来积累dout中的梯度(感觉还是没有理解的很透彻)
    #dW = np.add.at(dW, x, dout)（不能直接用等号，他是直接对dw进行in-palce的操作
    np.add.at(dW, x, dout)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i = sigmoid(a[:,:H])
    f = sigmoid(a[:,H:2*H])#记得打*这个符号，千万别忘了！！！
    o = sigmoid(a[:,2*H:3*H])
    g = np.tanh(a[:,3*H:])
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      next_h, next_c, cache_current = lstm_step_forward(x[:,t,:], current_h, current_c, Wx, Wh, b)
      h[:,t,:] = next_h
      current_h = next_h
      current_c = next_c
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      dcurrent_c = dprev_c
      dcurrent_h = dprev_h + dh[:,t,:]

      dx[:,t,:], dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dcurrent_h, dcurrent_c, cache[t])
