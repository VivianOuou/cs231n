...
def sim_positive_pairs(out_left, out_right):
    ...
    right_norm = torch.linalg.norm(out_right, dim = 1, keepdim = True)#(n,1)
...
def compute_sim_matrix(out):
    ...
    #计算out【i】与【j】之间的相似度
    sim_matrix = torch.mm(out, out.t()) / (norm * norm.t())#广播性质
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    #注意前一行的输出是（n,1),但是我们需要的是（2n,1)
    sim_pairs_pos = torch.cat([sim_pairs, sim_pairs],dim = 0)
    ...
    numerator = None
    numerator = torch.exp( sim_pairs_pos / tau )
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
