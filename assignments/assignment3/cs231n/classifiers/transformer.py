...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        #1. embed your caption and add positional encoding            #
        captions_embed = self.embedding(captions)
        captions_embed = self.positional_encoding(captions_embed)

        #2. project the image features(记得要保持同一维度，广播的性质不要忘记了)
        features_proj = self.visual_projection(features).unsqueeze(1)

        #3. prepare a mask (tgt_mask)
         #device = captions.device 保证张量都在同一个设备计算
         #torch.tril，转化为下三角矩阵，掩盖未来的信息，防止不准确
         #bool()，转换成布尔函数
        ...
        #5.Project the output to scores per token
